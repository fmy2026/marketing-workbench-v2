...
import { createJob, getJobView } from "../src/workflows/launchWorkflow.mjs";
...
const cleanupJobIds = [];
const effectiveJobId = jobId || (await (async () => {
  const view = await createJob(repo, {
    ...
    source_record_ref: `check:runtime-consistency:${new Date().toISOString()}`
  });
...
try {
  const bundle = await repo.getLaunchJobBundle(effectiveJobId);
  if (!bundle) throw new Error("job_not_found");
  const view = await getJobView(repo, effectiveJobId);
  const attempts = await repo.getCreateAttemptState(effectiveJobId);
  ...
  console.log(JSON.stringify({
    ...
    mode: jobId ? "existing_job" : "ephemeral_test_job",
  ...
  for (const cleanupJobId of cleanupJobIds.reverse()) {
    await repo.deleteTestJobCascade(cleanupJobId);
  }
}
