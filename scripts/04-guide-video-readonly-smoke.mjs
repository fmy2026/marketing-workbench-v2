...
async function run(guideVideoIds, { required = true } = {}) {
...
assert(unique.resourceWrites.length === 2, "guide_video_readiness_must_bind_to_each_required_video");
assert(unique.resourceWrites.every((item) => item.resourceMetadata?.guide_video_readiness?.guide_video_id === GUIDE_VIDEO_ID), "all_video_resources_must_use_the_unique_guide_video_id");
assert(unique.resourceWrites.every((item) => item.resourceMetadata?.guide_video_readiness?.verified_by_job_id === bundle().job.job_id), "guide_video_readiness_must_bind_to_current_job");
...
console.log(JSON.stringify({
  ...
  boundVideoCount: unique.resourceWrites.length,
