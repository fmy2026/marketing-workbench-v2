...
const state = {
  ...
  report: null, discoveryPage: 1, busy: false, modelSaving: false, ids: [], selectedId: "", activeModule: "conversation", currentUser: null, pendingQuestion: ""
};
const errors = {
  ...
  model_config_changed: "配置已在其他页面更新，请重新加载后再操作。"
};
...
function responseSource({ parseSource = "rules", aiStatus } = {}) {
  const request = parseSource === "model" ? "需求已使用大模型解析" : parseSource === "model_fallback" ? "大模型解析需求未成功，已回退规则解读" : "已使用规则解读";
  if (aiStatus === undefined) return `${request}。`;
  if (aiStatus === "used") return parseSource === "model" ? "需求已使用大模型解析；内容已使用大模型解析。" : `${request}；内容已使用大模型解析。`;
  if (["invalid_output", "unavailable"].includes(aiStatus)) return parseSource === "model" ? "需求已使用大模型解析；内容的大模型解析未成功，已回退规则解读。" : "大模型解析未成功，已回退规则解读。";
  return parseSource === "model" ? "需求已使用大模型解析；内容已使用规则解读。" : `${request}。`;
}
function sourcedReply(reply, source) { return `${responseSource(source)} ${reply || "已完成。"}`; }
...
async function loadDiscovery(page, { pending = null, reply = "已读取当前候选素材。", parseSource = "rules" } = {}) {
  if (!pending) setBusy(true, "正在读取素材");
  try { renderDiscovery(await api("/discover", { body: { page } })); finishTurn(pending, sourcedReply(reply, { parseSource })); }
...
async function loadSearch(filters, { pending = null, reply = "已按当前条件读取素材。", parseSource = "rules" } = {}) {
  if (!pending) setBusy(true, "正在核验观察范围");
  try { renderSearch(await api("/search", { body: { filters } })); finishTurn(pending, sourcedReply(reply, { parseSource })); }
...
function renderReport(report, { parseSource = report.parseSource || "rules" } = {}) {
  report.parseSource = parseSource; state.report = report; state.filters = { ...report.filters, page: state.filters?.page || 1, candidatePage: state.filters?.candidatePage || 1 }; const wrap = workspaceBase(); const layout = el("section", "mi-report"); const toolbar = el("div", "mi-report-toolbar"); const left = el("span", "mi-small", responseSource({ parseSource, aiStatus: report.aiStatus })); const actions = el("div", ""); const back = button("返回素材"); const edit = button("编辑摘要"); const download = button("下载 HTML", "mi-primary-button"); actions.append(back, edit, download); toolbar.append(left, actions); const paper = reportPaper(report); layout.append(toolbar, paper); const nudge = modelNudge(report.aiStatus); wrap.append(layout); if (nudge) wrap.append(nudge); $("workspace").replaceChildren(wrap); back.addEventListener("click", () => loadSearch(state.filters)); edit.addEventListener("click", () => openSummaryEditor(report, paper)); download.addEventListener("click", () => downloadReport(report)); renderSideModules();
}
...
async function loadReport(filters, { pending = null, reply = "已按当前研究对象整理月报。", parseSource = "rules" } = {}) { if (!pending) setBusy(true, "正在整理报告"); try { const report = await api("/report", { body: { filters } }); renderReport(report, { parseSource }); finishTurn(pending, sourcedReply(reply, { parseSource, aiStatus: report.aiStatus })); } catch (error) { const text = errorMessage(error); finishTurn(pending, text); renderStatus(text, { action: { label: "返回素材", run: () => state.filters && loadSearch(state.filters) } }); } finally { setBusy(false); } }
...
function comparableModelApiBase(value) { return String(value || "").trim().replace(/\/$/, ""); }
function modelHasUnsavedChanges() { return Boolean($("modelApiKey").value.trim()) || comparableModelApiBase($("modelApiBase").value) !== comparableModelApiBase(state.model?.apiBase) || $("modelName").value.trim() !== String(state.model?.modelName || "") || $("modelEnabled").checked !== (state.model?.enabled === true); }
function formatModelTime(value) { if (!value) return ""; const date = new Date(value); return Number.isNaN(date.getTime()) ? "" : new Intl.DateTimeFormat("zh-CN", { timeZone: "Asia/Shanghai", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(date); }
function updateModelDirty() { $("modelDirty").hidden = state.modelSaving || !modelHasUnsavedChanges(); }
function setModelSaving(value, status = "") { state.modelSaving = value; $("modelForm").querySelectorAll("input, button").forEach((node) => { node.disabled = value; }); if (status) $("modelState").textContent = status; updateModelDirty(); }
function modelStateText(config) { if (!config?.credentialConfigured) return "尚未保存 API Key。"; if (config.testStatus === "passed") return `已保存 · 测试通过 · ${config.enabled === true ? "已启用" : "未启用"}${formatModelTime(config.testedAt) ? ` · 最近测试 ${formatModelTime(config.testedAt)}` : ""}`; if (config.testStatus === "failed") return "已保存 · 最近测试未通过 · 未启用"; return "已保存 · 尚未测试 · 未启用"; }
function updateModel(config, { status = "" } = {}) { state.model = config; $("modelApiBase").value = config?.apiBase || ""; $("modelName").value = config?.modelName || ""; $("modelEnabled").checked = config?.enabled === true; $("modelKeyState").textContent = config?.credentialConfigured ? "API Key 已保存，留空则沿用。" : "请填写 API Key。"; $("modelState").textContent = status || modelStateText(config); updateModelDirty(); renderSideModules(); }
async function refreshModelConfig({ status = "" } = {}) { const config = (await api("", { root: MODEL_ROOT })).config; updateModel(config, { status }); return config; }
async function refreshSettings() { updateConnection(await api("/connection")); await refreshModelConfig(); }
async function handleMessage(text) {
  ...
  try {
    ...
    if (response.needsConnection) { state.pendingQuestion = text; finishTurn(pending, sourcedReply(response.reply || "请先连接公共电脑。", { parseSource: response.parseSource })); renderStatus(response.reply || "请先连接公共电脑。", { action: { label: "配置数据连接", run: () => openSettings("connection"), primary: true } }); await openSettings("connection"); }
    else if (response.intent?.kind === "discover") await loadDiscovery(response.discoveryPage, { pending, reply: response.reply || "正在读取当前已采集的候选素材。", parseSource: response.parseSource });
    else if (response.intent?.kind === "search") await loadSearch(response.filters, { pending, reply: response.reply || "已按当前条件查询素材。", parseSource: response.parseSource });
    else if (response.intent?.kind === "report") await loadReport(response.filters, { pending, reply: response.reply || "正在整理当前研究对象的月报。", parseSource: response.parseSource });
    else if (response.intent?.kind === "detail" && response.detail) { showDetail(response.detail); finishTurn(pending, sourcedReply(response.reply || "已打开素材详情。", { parseSource: response.parseSource })); }
    else if (response.intent?.kind === "trend" && response.trend) { renderTrend(response.trend, response.intent.assetId); finishTurn(pending, sourcedReply(response.reply || "已显示素材趋势。", { parseSource: response.parseSource })); }
    else if (response.intent?.kind === "insight") await loadInsight(response.intent.assetId, { pending, reply: response.reply || "已根据当前证据整理解读。", parseSource: response.parseSource });
    else { renderClarification(response); finishTurn(pending, sourcedReply(response.reply || "请先查看当前已采集的游戏和素材。", { parseSource: response.parseSource })); }
...
$("modelForm").addEventListener("input", updateModelDirty); $("modelEnabled").addEventListener("change", updateModelDirty);
$("modelForm").addEventListener("submit", async (event) => { event.preventDefault(); if (state.modelSaving) return; $("modelError").textContent = ""; const apiBase = $("modelApiBase").value.trim(); const modelName = $("modelName").value.trim(); const apiKey = $("modelApiKey").value; const enableAfterTest = $("modelEnabled").checked; if (/\/chat\/completions\/?$/i.test(apiBase)) { $("modelError").textContent = errors.invalid_model_api_base; return; } setModelSaving(true, "正在保存配置…"); let saved = false; try { const value = await api("", { root: MODEL_ROOT, method: "PUT", body: { api_base: apiBase, model_name: modelName, api_key: apiKey, enabled: false } }); saved = true; $("modelApiKey").value = ""; updateModel(value.config, { status: "配置已保存，正在测试连接…" }); const tested = await api("/test", { root: MODEL_ROOT, body: { configuration_updated_at: value.config.updatedAt, enabled: enableAfterTest } }); updateModel(tested.config, { status: "正在确认状态…" }); const confirmed = await refreshModelConfig(); if (confirmed.testStatus !== "passed" || confirmed.enabled !== enableAfterTest) throw { code: "model_config_changed" }; $("modelState").textContent = `配置已保存，测试通过，模型${confirmed.enabled ? "已启用" : "未启用"}。${formatModelTime(confirmed.testedAt) ? `最近测试 ${formatModelTime(confirmed.testedAt)}。` : ""}`; } catch (error) { if (!saved) { $("modelError").textContent = `保存失败：${errorMessage(error)}`; } else if (error?.code === "model_connection_test_failed") { try { await refreshModelConfig({ status: "配置已保存，但测试失败，模型未启用。" }); } catch { $("modelState").textContent = "暂时无法确认结果。"; } $("modelError").textContent = errorMessage(error); } else if (error?.code === "model_config_changed") { try { await refreshModelConfig({ status: "配置已保存，但配置已在其他页面更新，请重新加载后确认。" }); } catch { $("modelState").textContent = "暂时无法确认结果。"; } $("modelError").textContent = errorMessage(error); } else { try { await refreshModelConfig({ status: "配置已保存；测试结果暂未确认。" }); } catch { $("modelState").textContent = "暂时无法确认结果。"; } $("modelError").textContent = errorMessage(error); } } finally { setModelSaving(false); } });
