...
function updateModel(config) { state.model = config; $("modelApiBase").value = config?.apiBase || ""; $("modelName").value = config?.modelName || ""; $("modelEnabled").checked = config?.enabled === true; $("modelState").textContent = config?.credentialConfigured ? `测试状态：${config.testStatus === "passed" ? "已通过" : "未通过或未测试"}` : "尚未配置 API Key"; renderSideModules(); }
