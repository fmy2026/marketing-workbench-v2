...
| 状态字段 | 状态类字段均为 `text` |
| 复杂摘要 | 使用 `jsonb` |
| 私密信息 | 私密 URL、token、Cookie、raw payload、raw response 不入普通表 |
...
| `mwb.account_touchpoints` | 触点 ref 和 URL hash，不保存完整 URL |
...
| `mwb.account_resources` | 账户级头像、DMP、事件、视频、产品图、品牌、小程序可用性 |
| `mwb.material_packs` | 保底物料包 |
| `mwb.material_pack_items` | 保底物料包明细 |
| `mwb.launch_jobs` | 一次投放创建任务 |
| `mwb.launch_node_runs` | 7 个 Workflow 节点状态 |
