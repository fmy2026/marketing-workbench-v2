...
浏览器的视频 URL 指向已登录工作台自己的 `/api/agents/market_intelligence/files/{id}`，由后端携带本人 Token 只读访问公共服务；不提供含 Token 的分享链接，不缓存视频或原始响应。先发送“有哪些游戏”或“有哪些已采集素材”，页面会显示当前公共电脑候选中实际返回的游戏名称和素材；这不是全量游戏目录，可继续发现更多候选。再点击页面出现的游戏名称，或输入该名称查询目标月份素材并生成月报。未指定名称的“竞品”会要求补充，不会自动编造名单。模型 API Base、名称、Key、测试和启用沿用现有每用户每 Agent 配置：它是自由表达理解和创意观察的可选增强，未配置时仍返回真实素材、趋势、确定性证据说明和事实月报。真实连接需要公共电脑持续运行，IP 变化后在页面重新配置。模型/接口 Schema 测试与真实素材的带凭证查询、MP4/WebM 播放分别验收，不能混为成功。
