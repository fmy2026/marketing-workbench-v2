...
浏览器的视频 URL 指向已登录工作台自己的 `/api/agents/market_intelligence/files/{id}`，由后端携带本人 Token 只读访问公共服务；不提供含 Token 的分享链接，不缓存视频或原始响应。可问“看看 8 月巨兽战场的素材”“看看 8 月巨兽战场和荒野堡垒（竞品）的素材”“下一页”“生成 8 月巨兽战场的市场情报月报”；未指定名称的“竞品”会要求补充，不会自动编造名单。模型 API Base、名称、Key、测试和启用沿用现有每用户每 Agent 配置：修改连接后先保存、测试通过后才可启用。真实连接需要公共电脑持续运行，IP 变化后在页面重新配置。模型/接口 Schema 测试与真实素材的带凭证查询、MP4/WebM 播放分别验收，不能混为成功。
