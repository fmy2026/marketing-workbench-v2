...
追加视频改动发布后，额外检查一次“检查推送结果”或“重新只读准备”：页面应在运行结束后立即显示服务端的唯一卡点或确认卡，而不是停留在“正在核验”。对已受理但尚未确认的素材推送，必须使用“检查推送结果”回查原冻结视频；该检查只读，不点击确认、素材推送或追加按钮。
...
登录后从 Agent 广场进入“市场情报”。页面沿用 Agent 顶部栏和六模块导航，默认对话状态只有一个自然语言输入框；右上角“数据连接”和“大模型配置”分别打开配置面板。数据连接填写公共电脑 HTTP(S) 私网 IPv4 origin 和安全交接的 Token，不接受路径、查询参数、URL 内凭据、域名或重定向。HTTP 联调需在配置表单中明确确认明文传输及只读用途，随后执行健康接口验证再保存。空 Token 只在地址未变化时沿用已有配置；更换地址必须重新录入，防止旧凭证被转发到新服务。
...
浏览器的视频 URL 指向已登录工作台自己的 `/api/agents/market_intelligence/files/{id}`，由后端携带本人 Token 只读访问公共服务；不提供含 Token 的分享链接，不缓存视频或原始响应。先发送“有哪些游戏”或“有哪些已采集素材”，页面会显示当前公共电脑候选中实际返回的游戏名称和素材；这不是全量游戏目录，可继续发现更多候选。再点击页面出现的游戏名称，或输入该名称查询目标月份素材并生成月报。未指定名称的“竞品”会要求补充，不会自动编造名单。模型 API Base、名称、Key、测试和启用沿用现有每用户每 Agent 配置：它是自由表达理解和创意观察的可选增强，未配置时仍返回真实素材、趋势、确定性证据说明和事实月报。市场情报使用“保存并测试”一次完成：先保存当前输入、再测试该版本，成功后按勾选状态启用或停用；配置变更、网络中断和其他页面并发更新均会明确显示最终可确认状态。Base 填服务根地址（如 `https://…/v1`），不要填写 `/chat/completions`。真实连接需要公共电脑持续运行，IP 变化后在页面重新配置。模型/接口 Schema 测试与真实素材的带凭证查询、MP4/WebM 播放分别验收，不能混为成功。
