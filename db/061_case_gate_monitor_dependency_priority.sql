...
-- Scope: make the Case root blocker follow actual workflow dependencies.
-- Safety: projection only; no platform call and no Case, Job, Plan or resource mutation.
...
LEFT JOIN LATERAL (
  ...
  FROM (
    ...
    WHERE blocker.value <> ''
    UNION ALL
    SELECT monitor.blocker, 1, 1
    FROM (
      SELECT b.blocker
      FROM mwb.v_monitor_provision_blocker_report b
      WHERE b.route_id = wc.route_id AND b.game_code = wc.game_code AND b.advertiser_id = wc.advertiser_id AND b.blocker <> ''
...
COMMENT ON VIEW mwb.workflow_case_summary IS
  'Single current workflow gate. Root blocker selection follows confirmed action, monitor/context, resource, then Plan fallback dependency order.';
