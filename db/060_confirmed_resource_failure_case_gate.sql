...
-- Scope: surface a confirmed resource-plan stop as the single current Case root blocker.
-- Safety: projection only; no platform call and no resource, Plan, confirmation or action mutation.
...
SELECT
  ...
  CASE
    ...
    WHEN coalesce(attempt.std_project_create_action_count, 0) = 1
      AND coalesce(attempt.created_object_count, 0) = 1
      AND coalesce(attempt.last_readback_status, '') = 'readback_verified'
      THEN 'first_std_project_create_completed'
...
LEFT JOIN LATERAL (
  ...
  FROM (
    SELECT blocker.value AS blocker_code, 0 AS source_priority, blocker.ordinality::int AS item_priority, 0 AS detail_priority
    FROM (
      SELECT sr.blockers
      FROM mwb.launch_skill_runs sr
      WHERE sr.job_id = latest.job_id
        AND sr.skill_key = 'confirmed-resource-orchestrator'
        AND sr.status = 'blocked'
      ORDER BY coalesce(sr.finished_at, sr.started_at) DESC, sr.skill_run_id DESC
      LIMIT 1
    ) confirmed_resource
    ...
    SELECT blocker.value AS blocker_code, 1 AS source_priority, 8 AS item_priority, 0 AS detail_priority
    FROM mwb.launch_skill_runs sr
...
COMMENT ON VIEW mwb.workflow_case_summary IS
  'Single current workflow gate. A verified single std-project creation closes the Case before a retained ready Plan can request another authorization; root_blocker_codes contains zero or one blocker.';
