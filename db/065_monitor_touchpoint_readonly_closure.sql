...
-- Scope: make a monitor touchpoint READY only after the readonly reconcile has
--        persisted and verified it, and make run_monitor_readonly expose the
--        canonical monitor blocker. No platform call or run-history deletion.
...
SELECT
  ...
  (
    ...
    AND provision_status IN ('touchpoint_resolved', 'resolved')
  ...
  CASE
    ...
    WHEN monitor_id_present AND provision_status NOT IN ('touchpoint_resolved', 'resolved') THEN 'needs_touchpoint_readback'
  ...
  CASE
    ...
    WHEN monitor_id_present AND provision_status NOT IN ('touchpoint_resolved', 'resolved') THEN 'touchpoint_url_hash_mismatch'
  ...
  CASE
    ...
    WHEN monitor_id_present AND provision_status NOT IN ('touchpoint_resolved', 'resolved') THEN 'run_monitor_readonly_reconcile'
...
LEFT JOIN LATERAL (
  ...
  FROM (
    SELECT monitor.actionable_blocker_code AS blocker_code,
      CASE WHEN monitor.readiness_status IN ('needs_readonly', 'needs_touchpoint_readback') THEN -1 ELSE 1 END AS source_priority,
      1 AS item_priority
    WHERE monitor.actionable_blocker_code <> ''
    ...
    WHERE sr.job_id = latest.job_id AND sr.skill_key IN ('context-resolve-account', 'context-resolve-touchpoint') AND blocker.value <> ''
      AND blocker.value NOT IN ('monitor_id_missing', 'touchpoint_url_missing', 'touchpoint_url_hash_mismatch')
    UNION ALL
...
COMMENT ON VIEW mwb.workflow_case_summary IS
  'Single current workflow Gate. run_monitor_readonly exposes the canonical monitor actionable blocker before stale skill evidence.';
