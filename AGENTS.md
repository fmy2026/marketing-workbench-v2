...
| 校验基线 | Git `4c0a737` + 当前终态 Monitor readonly Task；`project.state.json.schema_version=2026-08-28.project-control-plane-v2` |
...
?case_id=：恢复活动 Case 的最新 Job，可继续受 Gate 约束的流程。
?job_id=：仅历史只读查看，不得确认或执行。
```
...
- 对 `monitor_create_busy_retry_exhausted` 的终态 Case，工作台只接受精确“重新只读回查 monitor”触发一次 fresh readonly reconcile；该动作不改变 Gate 真值，也不授权创建或重试。
