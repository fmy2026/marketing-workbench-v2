...
- 每份确认 Plan 只能按冻结动作消费一次；失败或修正必须使用新 Plan、hash、confirmation 和 attempt。唯一例外是新冻结且明确携带重投合同的 `std_project_create` 或 `oc_project_video_append` action 收到无对象/受理结果的精确 `HTTP 200 + 40100` 后，可依同一 confirmation 在动作内最多三次错峰物理投递；每笔请求 hash 必须相同，其他错误、超时或结果不明一律禁止自动重试。
