...
| 版本 | 更新时间 | 变更内容 |
|------|----------|----------|
| v1.1（当前） | 2026-09-10 | 新增素材标签下拉列表、上传素材文件、上传素材三个接口 |
...
| companyList | 公司主体列表 |
| resourcePurpose | 素材用途列表（上传素材时的 use_type） |
| resourceProduct | 素材所属产品列表 |
| resourceLicense | 素材备案状态列表 |
| resourceDirection | 素材方向列表 |
| resourceLabel | 素材常规标签列表 |
| resourceStyle | 素材画面风格列表 |
| resourcePlay | 素材玩法列表 |
| resourceFormat | 素材媒体位置/格式列表 |
...
获取素材库列表，支持按素材类型（图片/视频/落地页/图文/音频/组图）分页查询。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| resource_type | int | 是 | 素材类型：1=图片，2=视频，3=落地页，4=图文，5=音频，7=组图 |
| pageNo | int | 否 | 页码，默认 1 |
| pageSize | int | 否 | 每页条数 |
...
上传图片/视频/音频文件到 CDN，返回素材链接和 MD5 等，供「上传素材」接口组装 `material` 时使用。

> 说明：此接口**仅支持 API Token 访问**，普通网页登录方式调用会被拒绝。

- **路径**: `/resource/resource/upload_file`
- **方法**: `POST`
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| resource_type | int | 是 | 素材类型：1=图片，2=视频，5=音频 |
| file | file | 是 | 上传的文件 |

**支持的文件格式**：

| resource_type | 支持的 MIME |
|------|------|
| 1（图片） | image/gif、image/jpeg、image/jpg、image/png |
| 2（视频） | video/mp4、video/avi |
| 5（音频） | audio/mpeg、audio/mp4 |
...
```bash
curl -X POST 'https://center-test.3kwan.com/resource/resource/upload_file' \
  -H 'X-Passport-Token: your-api-token-here' \
  -F 'resource_type=1' \
...
```json
{
  ...
  "msg": "success",
  "data": {
    "url": "https://cdn.3k.com/resource/xxx/image.png",
    "resource_md5": "e10adc3949ba59abbe56e057f20f883e",
    "width": 0,
    "height": 0
  }
}
...
| 字段 | 类型 | 说明 |
|------|------|------|
| url | string | 素材 CDN 地址，用于上传素材接口的 `material[].url` |
| resource_md5 | string | 文件 MD5，用于上传素材接口的 `material[].resource_md5` |
| width | int | 视频宽度（图片/音频返回 0） |
| height | int | 视频高度（图片/音频返回 0） |

---

### 16. 上传素材

新增素材，支持图片、视频、落地页、图文、音频、组图等类型。

- **路径**: `/resource/resource/add_resource`
- **方法**: `POST`
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| resource_type | int | 是 | 素材类型：1=图片，2=视频，3=落地页，5=音频 |
| use_type | int | 是 | 用途 ID（对应 `resourcePurpose` 列表） |
| material | array | 是 | 素材列表，每个元素字段见下表 |

**material[] 元素字段**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| url | string | 是 | 素材 CDN 地址（可用「上传素材文件」接口获取） |
| resource_name | string | 是 | 素材名称 |
| product_id | int | 是 | 所属产品 ID（对应 `resourceProduct` 列表） |
| maker | array | 是 | 制作人 SSO 用户 ID 列表 |
| license_id | int | 是 | 备案状态 ID（对应 `resourceLicense` 列表） |
| resource_md5 | string | 否 | 文件 MD5 |
| direct_id | array | 否 | 方向 ID 列表（对应 `resourceDirection`） |
| label_id | array | 否 | 常规标签 ID 列表（对应 `resourceLabel`） |
| play_id | array | 否 | 玩法 ID 列表（对应 `resourcePlay`） |
| style_id | array | 否 | 画面风格 ID 列表（对应 `resourceStyle`） |
| format_tag_id | array | 否 | 媒体位置/格式 ID 列表（对应 `resourceFormat`，仅推广场景生效） |
| content_element | array | 否 | 内容元素，传 `[{"label":"元素名称"}]`（无需 ID，后端按名称自动建档） |
| asset | array | 否 | 画面元素，传 `[{"label":"元素名称"}]`（无需 ID，后端按名称自动建档） |

> 提示：视频宽高由后端根据 `url` 自动计算，无需手动传 `width`/`height`。

**请求示例（图片）**：

```bash
curl -X POST 'https://center-test.3kwan.com/resource/resource/add_resource' \
  -H 'X-Passport-Token: your-api-token-here' \
  -d 'resource_type=1' \
  -d 'use_type=10' \
  -d 'material[0][url]=https://cdn.3k.com/resource/xxx/image.png' \
  -d 'material[0][resource_name]=宣传图片A' \
  -d 'material[0][product_id]=100' \
  -d 'material[0][maker][]=zhangsan' \
  -d 'material[0][license_id]=20' \
  -d 'material[0][resource_md5]=e10adc3949ba59abbe56e057f20f883e' \
  -d 'material[0][content_element][0][label]=内容元素A' \
  -d 'material[0][asset][0][label]=画面元素A'
```
...
> 说明：
> - 图文（resource_type=4）、组图（resource_type=7）提交结构不同：图文需 `product_id` + `image_list`（引用已存在素材标识码）；组图需 `sub_type` + `image_list`。建议先通过「获取素材库列表」取得已有素材后再提交。
> - 各标签 ID 请通过「获取系统基础下拉列表」接口（`/ajax/selectList/getList`）的 `resourceXxx` 类型获取。
