...
| 追加视频 `40100` 有界错峰投递 | 新冻结的 `project_video_append` Plan 将端点、`40100`、三次调用点、抖动与总时限同时固定到 Plan、scope 和 planned action；仅精确 `HTTP 200 + code=40100` 才按相同 wire/hash 再投递，最多三次。其他结果停止写入并只回查；三笔 delivery 仍是一个 Case action | [本次批准 Task](../tasks/TASK-MWBV2-APPEND-RATE-LIMIT-RECOVERY-20260915.md)、本地官方频控文档、[当前逻辑](project-现在的逻辑图.md) |
