...
| 统一 LaunchRequest 与澄清 | 页面自然语言和站内 JSON 都归一为 `launch-request.v1`，本期唯一事项是 JSZC 的 `create_std_project`。JSON 严格校验且绝不调用模型；自然语言以规则优先、模型仅凭明示证据补空槽位。页面内草稿支持分次补齐和明确纠正，多账户、不支持事项或解析失败会清除不可启动槽位。Case 与 Job 使用同一已冻结请求，用户身份、账户归属、Plan、确认、单次执行和回查不从请求中取得授权 | [本次批准 Task](../tasks/TASK-MWBV2-LAUNCH-REQUEST-UNIFIED-INTAKE-20260913.md)、[当前逻辑图](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
