...
| 元信息 | 值 |
| --- | --- |
| 文档性质与状态 | 当前有效；方案索引与批准选择 |
| 用途与范围 | 说明为什么采用当前路线，以及哪些关键选择已获批准 |
| 权威依据 | 已批准 Task、当前逻辑图、数据与报表契约；启动和权限规则查 AGENTS |
| 最后更新 | 2026-09-21 CST |
| 核验范围 | 静态核对当前决策入口及与逻辑、数据、部署文档的职责边界；不证明业务运行状态 |
| 更新条件 | 方案方法、Agent 职责或已批准关键选择变化时更新 |

本文回答“为什么这样选择”。当前行为分别查[逻辑图](project-现在的逻辑图.md)、[数据与报表契约](project-数据与报表契约.md)和[部署说明](../deploy/README.md)；启动、权限和任务闭环只查[AGENTS](../AGENTS.md)。阅读入口见[文档导航](README.md)。
...
已批准 `TASK-MWBV2-MI-SEARCH-REPORT-20260920`：市场情报在同一工作区提供“问题 → 已采集样本 → 证据解读或 HTML 月报”的只读闭环。公共电脑是事实所有者；模型仅在本人测试启用后解析未识别需求或改写脱敏证据。它不建立情报库、报告历史、Case、Plan 或平台动作。运行流程见[逻辑图](project-现在的逻辑图.md#7-市场情报-agent只读证据链与演进边界)，字段、数量和接口限制见[数据契约](project-数据与报表契约.md#市场情报外部只读合同)，连接与真实联调操作见[部署说明](../deploy/README.md#市场情报数据连接)。

代码与模拟验证已完成；真实公共电脑查询、MP4/WebM、已启用模型和 HTML 导出仍待获授权用户联调。该 Task 按 cancelled 关闭，缺口以其[Context Manifest](../tasks-context-manifests/TASK-MWBV2-MI-SEARCH-REPORT-20260920.json)为准。素材比较、全量月度覆盖、报告历史、自动月报和投放策略仍须独立批准与验收。

## 已批准：文档架构整理（2026-09-21）

当前文档按“方案、流程、数据、部署”单一职责维护，新增[文档导航](README.md)只提供阅读入口。后续新增 Agent 先在本文件记录需要人工确认的选择，再依 AGENTS 建立独立 Task；代码目录、路由拆分、注册入口统一和测试入口补齐均为后续独立实现，不视为已完成。
...
| 决策 | 已批准选择与理由 | 依据 / 当前合同 |
| --- | --- | --- |
| Agent 开发经验复盘 | 按历史任务证据提炼面向软件 Agent 的人机协作图谱：最小闭环、整体对齐、知识核验、模块变化维度及数据报表思考；历史事实、当前机制与重做建议分别表达。保留八项核心事项与权威文件映射，补充产品运行设计，按路径与动作后果选择架构，长任务、多 Agent 等能力按条件叠加；数据库和报表按需建设。作为方法参考，不替代现有项目合同，也不改变运行行为 | [批准任务](../tasks/TASK-MWBV2-AGENT-DEVELOPMENT-EXPERIENCE-MAP-20260916.md)、[清单补充任务](../tasks/TASK-MWBV2-MINIMAL-AGENT-DEVELOPMENT-DOCS-20260916.md)、[运行设计补充任务](../tasks/TASK-MWBV2-AGENT-RUNTIME-DESIGN-GUIDANCE-20260916.md)、[方法图谱](Agent项目从零开发-人机协作经验图谱.md) |
...
| 启动入口与发布凭据 | 完整 Intake 仅在最新对话下显示一张启动卡片；点击冻结服务端已校验的完整请求，并以同一快照创建 Case 与 Job，先显示真实启动阶段，取得 Job 后才展示服务端七节点。追加视频的项目素材只读查询使用官方 `filtering.material_type`，素材标识码以原始大小写作精确边界匹配；目标账户缺少但物料户唯一命中的视频先冻结独立推送 Plan，`video_ids` 全链保持原始非空字符串及大小写。追加调用使用无损 JSON 整数 token 传输账户和项目 ID，视频 ID 保持字符串；Plan、临近执行和请求体共用同一 wire 合同。推送平台动作和权威库存回查分别记录；只有回查通过才在同一 Job 自动重跑只读准备，过滤项目已有视频并生成独立追加 Plan。确认后的追加进入专用只读回查 Gate，旧 Plan 不会重发；仅当平台明确拒绝且回查仍缺失时，可建立同一 Case 的一个 fresh 追加恢复 Job。确认卡展示本次核验、项目已有和待追加数量；右侧仅固定显示事项和 3 阶段 7 节点，动态状态留给节点、顶部和对话。对话轮次在当前页面内保留，输入、快捷入口和确认均呈现用户决策及原位完成的规则/模型解析回复。固定 release 通过 LaunchAgent 的绝对共享凭据路径读取乾坤授权，切换前只核验文件可读，不复制或输出凭据 | [本次批准 Task](../tasks/TASK-MWBV2-WORKBENCH-STARTUP-REPAIR-20260914.md)、[当前逻辑图](project-现在的逻辑图.md)、[部署说明](../deploy/README.md) |
| 确认卡、七节点进度与执行取证 | 资源确认卡同时冻结并展示业务数量、批次与调用上限；确认 claim 原子将 Plan 从 `ready` 切至 `executing`，执行器只接受精确已确认的 `executing` Plan。七节点完成数、当前节点和执行阶段由服务端单一投影；每轮执行与 Skill 均记录独立 cycle，等待时间不并入技能耗时 | [本次批准 Task](../tasks/TASK-MWBV2-WORKBENCH-PROGRESS-EXECUTION-OBSERVABILITY-20260913.md)、[当前逻辑图](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
| 已确认创建 Plan 的 Job 边界 | confirmation ID 由 `plan_id` 派生，数据库 claim 原子核验 latest Job、Plan/hash、Case 生命周期和本人范围。同一 Job 已确认 `std_project_create` 后，Node 01–05 不得重新准备、覆盖 Draft 或发布后续 Plan；只有该 Plan 的 Node 07 回查可继续。创建前零动作停止时保留具体 blocker 与脱敏观察引用，必须经同 Case 的 fresh readonly Job 重新生成 Plan/hash 并再次确认 | [本次批准 Task](../tasks/TASK-MWBV2-CONFIRMED-PLAN-RECOVERY-20260913.md)、[当前逻辑图](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
...
| 资源准备与回查 | 只为注册表支持的资源编译动作；事件资产创建响应有 ID 后只作有界精确 readonly 回查，绝不重发创建；视频准备合同从当前 Job 装载的物料户 verified 映射解析必需集合，统一冻结可执行性、缺失集合、批次和 hash。每条视频的显式封面只可使用同一 target 资源、同一 Job 的 visible/readback 证据；允许默认封面时明确省略。该解析结果由 Draft、Plan/hash、payload、嵌套合同和 preflight 共用。全部就绪为零动作，部分缺失只绑定该集合；来源缺失、歧义、未验证、查询失败或集合不完整均停止，配置与目标绑定保持既有顺序核验 | [事件资产 Task](../tasks/TASK-MWBV2-EVENT-ASSET-READBACK-DYNAMIC-VIDEO-20260911.md)、[视频来源 Task](../tasks/TASK-MWBV2-VIDEO-BIND-PLAN-SOURCE-RESOURCES-20260911.md)、[本次批准 Task](../tasks/TASK-MWBV2-UNIVERSAL-CREATE-CLOSURE-20260913.md)、[逻辑图](project-现在的逻辑图.md)、[资源能力注册表](../src/workflows/skills/oe3/04-resource-action-registry.mjs) |
...
| 资源动作精确调用量 | 资源 executor 的 fresh readonly 结果是该动作唯一调用量来源：0 表示已满足、不生成写动作；正整数同时冻结在 planned action、action grant 与 Plan 总调用量。视频额外冻结绑定集合 hash；确认和临近写入均重算同一合同。任一集合、hash、数量或授权不一致均在 confirmation claim 前 fail-closed，必须走 fresh Job/Plan，不能改写旧 Plan | [本次批准任务](../tasks/TASK-MWBV2-GENERIC-RESOURCE-ACTION-CALL-LIMIT-20260908.md)、[本次批准 Task](../tasks/TASK-MWBV2-VIDEO-PREP-RECOVERY-GUARD-20260913.md)、[当前逻辑](project-现在的逻辑图.md) |
...
| 追加视频有界投递与统一回查收口 | 新冻结的 `project_video_append` Plan 将端点、`40100`、三次调用点、抖动与总时限固定到 Plan、scope 和 planned action；仅精确 `HTTP 200 + code=40100` 且不存在受理/对象矛盾证据时，才按相同 wire/hash 再投递，最多三次。动作后的即时观察与后续只读观察都由同一事务保存 readback、脱敏 evidence、节点 05–07、Job 与 Case；只有项目素材集合全量命中才完成 | [本次批准 Task](../tasks/TASK-MWBV2-APPEND-UNIFIED-CLOSURE-20260915.md)、本地官方频控文档、[当前逻辑](project-现在的逻辑图.md) |
| Node 05 查重 `40100` 一次只读恢复 | 同名与语义 `std_project/list` 仅在首次 `HTTP 200 + api_code=40100` 时，以完全相同参数等待由 Job 确定的 `20–24` 秒后重试一次；成功即继续原查重，第二次 `40100` 以专属 blocker 停止，其他失败一律不重试。只记录调用次数、最终业务码与恢复标记；不产生 Plan、confirmation、action 或 Attempt | [本次批准 Task](../tasks/TASK-MWBV2-DUPLICATE-READONLY-40100-RECOVERY-20260911.md)、[当前逻辑](project-现在的逻辑图.md) |
...
| 引导视频能力自动识别 | JSZC 每个 fresh Job 在唯一已核验小游戏实例上仅作一次 `gameplay/list` readonly；唯一非空 ID 要求每条 required video，以及每条待追加视频，复用它，空列表允许省略，多候选或 probe 失败 fail-closed。账户 `guide_video_required=true` 仅为兼容性强制要求，不能将空结果降级；动态 ID 仅保存在当前 Job 绑定的资源 metadata。`platform_status` 不参与此分支 | [本次批准任务](../tasks/TASK-MWBV2-JSZC-GUIDE-VIDEO-AUTO-DETECT-20260909.md)、[当前逻辑](project-现在的逻辑图.md) |
...
| 初始只读核验入口 | `run_fresh_readiness` 只显示“开始只读核验”并提交“继续执行”；“重新只读准备”仅是受阻 Gate 的恢复入口。为兼容已显示的旧提示，前一 Gate 收到该文字命令时只重跑同一最新 Job 的 dry-run，不新建 Job、不重放 Plan、不确认或写平台 | [本次启动修复任务](../tasks/TASK-MWBV2-WORKBENCH-STARTUP-REPAIR-20260914.md)、[当前逻辑](project-现在的逻辑图.md) |
| corrective Attempt 的只读准备命令 | 当 `workflow_case_summary.current_gate=prepare_corrective_attempt` 时，“重新只读准备”与“继续执行”汇入同一既有 fresh corrective Attempt 分支；前者为工作台主提示，后者仅为兼容别名。该分支继续以 Case 锁、最新失败 Job、已消费 Plan、尝试上限和零创建对象为条件，不复制 Plan 或自动写平台 | [本次批准 Task](../tasks/TASK-MWBV2-GAME-BRAND-FALLBACK-VALIDATION-20260910.md)、[当前逻辑](project-现在的逻辑图.md) |
| 已确认创建前零动作恢复 | 已确认的 `std_project_create` Plan 在任何平台 action 前停止时，executor 先保存最具体的上游 blocker；summary 不以通用 readiness 包装原因覆盖它。仅该 Job 已消费、无 create action、无创建对象、次数未耗尽、Case/owner/latest 条件仍成立时，可复用既有“重新只读准备”入口创建或返回同一个 fresh Job；新 Job 必须重新核验、生成新 Plan/hash 并再次确认。已有对象、动作或结果不明仍只允许 Node 7 readonly，绝不借恢复重发创建 | [本次批准 Task](../tasks/TASK-MWBV2-UNIVERSAL-CREATE-CLOSURE-20260913.md)、[当前逻辑](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
| 确认创建跨执行轮次失败收口 | Skill 执行记录的唯一性以 `job_id + execution_cycle + skill_key + attempt_no` 为准；旧三字段约束必须移除。确认领取成功后若本地执行异常，收口事务锁住同一 Plan 与 confirmation，且只有同一 Job/Plan 不存在平台 action、投递、创建对象和运行中 cycle 时，才将 Plan 一次性消费为 `blocked_before_create` 并记录 `confirmed_create_execution_failed_before_action`；创建 action 认领在同一锁下重验 Plan 仍为 `executing`，旧确认不可重放。随后只能经现有 fresh readonly recovery 生成新 Job、Plan/hash 并再次确认。任一 action、投递、对象、运行中 cycle 或核验异常均保持关闭，转入既有回查或人工核验，不能推定零动作 | [本次批准 Task](../tasks/TASK-MWBV2-CREATE-CYCLE-FAILURE-CLOSURE-20260921.md)、[当前逻辑](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
| 确认循环恢复完整性 | 历史或竞态导致“V1 已确认零动作停止、同 Job 又有 V2 ready”时，migration `090` 仅将后发未确认 V2 标为 `stale`；`workflow_case_summary` 优先选择 V1 的实际停止结果，展示具体 blocker 与 `create_fresh_readonly_recovery`。页面只消费同一次服务端 `confirmationPreview`，显式 `null` 清除卡片，禁止客户端回退到旧 Plan | [本次批准 Task](../tasks/TASK-MWBV2-CONFIRMED-PLAN-RECOVERY-20260913.md)、migration `090`、[当前逻辑图](project-现在的逻辑图.md) |
...
| OAuth 自动维护与异常恢复 | 当前项目的单一 macOS LaunchAgent 在登录时和每小时第 1 分钟触发确定性维护脚本；距 access token 到期不足两小时才刷新。刷新、原子保存与 OAuth 已授权账户只读验证全部成功才标为 ready。DNS/连接失败留给下一周期；超时或结果不明禁止自动重发；验证失败只重做只读验证。状态和本机通知均脱敏，任务不依赖 Codex、模型或 active Task | [本次批准任务](../tasks/TASK-MWBV2-OCEANENGINE-TOKEN-MAINTENANCE-20260922.md)、[维护实现](../src/platforms/oceanengineTokenMaintenance.mjs)、[部署说明](../deploy/README.md) |

## 何时使用

调整流程、Node、Skill、API、数据库、View、报表、授权、外部接口或迁移时先形成方案；需要比较路径或人工关键决策时也使用本文件。普通文案和小范围可逆修正可压缩为“问题 → 推荐修正 → 验收 → 停止条件”，不为形式增加材料。

## 已停止：通用机制最小简化（2026-09-13）

用户批准四个串行 Task：①本机独立测试库与合成数据回归基线；②移除正式流程中的测试成功捷径与旧授权兼容；③统一资源动作定义和清理重复作用域；④合并仓储重复装配并整体验收。各 Task 单独建立、检查、验证、关闭并交付 main。

用户随后要求停止方案推进：Task 1 按 cancelled 关闭，保留已完成的隔离测试与服务内部装配改动；Task 2–4 未启动。既有分组测试结果与未完成的汇总验收见对应 Context Manifest，不视为四项方案已完成。

用户后来要求以核心验证继续推进。Task 2 已将正式 runner、Plan 编译、创建与回查从测试成功捷径中剥离；假平台只作为测试依赖注入，旧测试授权和单动作兼容范围不再构成正式授权路径。Task 3–4 继续按独立任务推进。

Task 3 将确认资源动作的惰性执行器装配并入资源动作注册表；编排器只负责按冻结 Plan claim、排序、停止和记录，不再维护第二份动作到执行器映射。

Task 4 将核心上下文与 Job Bundle 共用的账户、路线和游戏 SQL 作用域谓词收束为仓储内部片段，保留每个入口各自的字段投影、排序和单次查询。旧 workflow-case 专项夹具仍含已移除的 mock 就绪假设，后续只在夹具迁移任务中处理，不恢复正式旁路。

保留 Case/Job/Plan、3 阶段 7 Node、38 张表、7 个 View、HTTP 与确认合同。业务样本只作 Postgres 只读对照，不重新执行完成 Case。测试使用当前 Schema 的只读导出、合成夹具、独立数据库和假外部依赖；禁止业务库测试写入及真实平台调用。以已有业务断言、权限、调用次数和输出等价性验收，不用减少文件数代替机制简化。必要的执行前重新核验、单次确认、尝试上限及精确 40100 规则保留。

## 方案最小格式

## 已批准：追加恢复额度（2026-09-15）

同一 Case 的追加 action 跨 Job 最多三次。每次恢复都先完整只读准备并生成新的冻结 Plan/hash，由本人独立确认；系统不自动重发。只有明确平台拒绝且成功回查确认仍缺失时才可恢复；超时、网络异常、5xx、非 JSON、缺少业务码和查询失败只允许回查。服务端在冻结 wire/hash 后以 Case 锁原子领取额度，并要求距上次 action 结束至少 20 秒。

追加 action 的幂等键按冻结 Plan ID 和已核验素材集合生成：同一 Plan 的重复提交始终收敛为一次 action，新的 Job/Plan 即使追加同一素材也不会与历史 action 键冲突。平台动作表的顶层错误分类沿用受限枚举，明确拒绝的细分结论保存在受控 metadata，并由 Plan 收口与回查 Gate 消费。

## 已批准：项目视频追加（2026-09-14）

`append_project_videos` 是 `LaunchRequest v2` 的独立事项：一次只接受一个本人账户、一个项目和 1–100 个明确视频标识码。自然语言与 JSON 统一进入同一校验器；标识码不得由模型改写，重复、超限、来源不唯一和只读查询不完整均停止。

创建与追加共用 Case、冻结 Plan、本人确认、单次消费和权威回查。追加只允许新增已核验视频，禁止修改原素材、标题、预算、ROI、定向或项目启停。

### 已批准：追加视频只读反馈清晰化（2026-09-21）

追加视频的来源库存、目标库存与项目素材查询必须把“查询未通过”和“已通过但分页/项目身份合同异常”分开停止；只在现有 Plan `readonly_checks` 保存环节、页码、客户端状态、HTTP/API 状态、凭据 blocker 与响应指纹的脱敏投影。来源失败显示在 Node 3，目标库存或项目素材失败显示在 Node 4；顶端、对话和底部进度共同消费服务端 `workflow_case_summary` 的唯一 blocker。启动 readonly 的对话先显示正在核验，收到服务端最终 Job 投影后原位替换为当前 Gate；恢复仍只使用既有“重新只读准备”，不自动确认、推送或追加。此项不改变 OAuth、7 Node、Plan/confirmation/action grant 或数据库 Schema。

### 已批准：素材推送延迟回查收口（2026-09-21）

已确认的 `project_video_material_push` 只调用一次平台绑定。即时与后续恢复均按冻结 `video_ids` 查询目标账户；首次未可见时，Plan 保持已消费、Node 04 阻断，并在页面存活期间按相对受理时间 `0/10/20/30/60/120/180` 秒分轮触发短只读请求。每轮由原 Plan/hash 与轮次原子领取，关闭页面不会重置窗口；窗口结束或查询失败才提供“检查推送结果”的人工单轮入口。它不重新编制推送 Plan、不重新确认、不重发。每次观察以原 Job、Plan/hash、冻结批次与观察时间精确绑定，在同一事务保存脱敏 evidence、readback、Node 01–07 投影、Plan 和 Job；较早开始的迟到观察不得覆盖较新的结果。全部视频可见后，才重新核验项目素材并生成独立的追加确认卡。查询失败是未知状态，不能计为已核验或视频缺失。
