...
| 启动入口与发布凭据 | 完整 Intake 仅在最新对话下显示一张启动卡片；点击先显示真实启动阶段，取得 Job 后才展示服务端七节点。固定 release 通过 LaunchAgent 的绝对共享凭据路径读取乾坤授权，切换前只核验文件可读，不复制或输出凭据 | [本次批准 Task](../tasks/TASK-MWBV2-WORKBENCH-STARTUP-REPAIR-20260914.md)、[当前逻辑图](project-现在的逻辑图.md)、[部署说明](../deploy/README.md) |
