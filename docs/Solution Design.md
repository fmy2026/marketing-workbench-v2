...
| 资源准备与回查 | 只为注册表支持的资源编译动作；事件资产创建响应有 ID 后只作有界精确 readonly 回查，绝不重发创建；视频准备合同从当前 Job 装载的物料户 verified 映射解析必需集合，统一冻结可执行性、缺失集合、批次和 hash。全部就绪为零动作，部分缺失只绑定该集合；来源缺失、歧义、未验证、查询失败或集合不完整均停止，配置与目标绑定保持既有顺序核验 | [事件资产 Task](../tasks/TASK-MWBV2-EVENT-ASSET-READBACK-DYNAMIC-VIDEO-20260911.md)、[视频来源 Task](../tasks/TASK-MWBV2-VIDEO-BIND-PLAN-SOURCE-RESOURCES-20260911.md)、[本次批准 Task](../tasks/TASK-MWBV2-VIDEO-PREP-RECOVERY-GUARD-20260913.md)、[逻辑图](project-现在的逻辑图.md)、[资源能力注册表](../src/workflows/skills/oe3/04-resource-action-registry.mjs) |
