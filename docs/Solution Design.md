...
| 目标空品牌列表的整组省略验证 | 游戏维度候选已经历一次未定位字段的显式失败，不能作字段归因或重放。仅当 fresh 目标账户品牌接口成功返回空列表、Case JSON 有一次性授权且 route/game/Job 精确匹配时，才在新的单次 Create Plan 中整组省略 `brand_info`；不发送空或部分对象。非空列表使用完整目标账户品牌对象；回查失败、不完整或不明保持阻断。成功并权威回查后才可提升为通用 capability，明确失败或不明均不自动产生下一次尝试 | [本次批准 Task](../tasks/TASK-MWBV2-GAME-BRAND-FALLBACK-VALIDATION-20260910.md)、官方创建字段文档、[逻辑图](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
