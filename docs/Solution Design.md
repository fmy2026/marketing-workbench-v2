...
已批准 `TASK-MWBV2-MI-SEARCH-REPORT-20260920`：首版在同一市场情报工作区完成“提出问题 → 按研究对象和上海月份查找已采集素材 → 打开视频与趋势 → 生成并编辑 HTML 月报”。查询只把每个研究对象当前最多 40 条候选素材的目标月份有效趋势作为归属依据，尚未遍历完时保留继续查询入口；浏览页每页 8 条。月报只采用有效观察样本，最多 30 条并按研究对象轮流选取，按“三个重点、研究对象、代表素材、继续关注”展示；可编辑摘要并导出无外部依赖的 HTML。模型只能在规则无法理解问题时提出有原文证据的研究对象、月份和目的，或基于脱敏样本形成三条观察；数值、样本数、观察范围和证据关联始终由服务端计算。未配置或调用失败时保留确定性事实摘要，不建立情报库、报告历史或长期记忆。
