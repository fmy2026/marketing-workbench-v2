...
| 品牌保底候选验证 | 新鲜目标账户品牌/行业回查始终优先。游戏维度候选仅保留为已消费历史 Plan 的解释证据，不能进入新 Draft 或被重放；多版本、过期、范围或 hash 不匹配一律阻断 | [本次批准 Task](../tasks/TASK-MWBV2-GAME-BRAND-FALLBACK-VALIDATION-20260910.md)、[逻辑图](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
| 目标空品牌列表的整组省略验证 | 仅当 fresh 目标账户品牌接口成功返回空列表、Case JSON 有一次性授权且 route/game/Job 精确匹配时，才在新的单次 Create Plan 中整组省略 `brand_info`；不发送空或部分对象。`not_required/not_required` 必须同时具有有效只读证据，并由 Node 04 的唯一就绪谓词被 Node 05、嵌套合同和 Execution Plan 共同消费；品牌权限仍由 `brand_mode` 的完整范围合同决定。字段形态使用同一纯校验：仅把精确五条品牌省略账本记录临时投影为历史四字段品牌形态并与既有基线比较，任何品牌外差异、额外路径或残缺省略均阻断；真实 Draft、Plan 和请求保留事实上的省略账本。非空列表使用完整目标账户品牌对象；回查失败、不完整或不明保持阻断。成功并权威回查后才可提升为通用 capability，明确失败或不明均不自动产生下一次尝试 | [本次批准 Task](../tasks/TASK-MWBV2-GAME-BRAND-FALLBACK-VALIDATION-20260910.md)、官方创建字段文档、[逻辑图](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
...
| 标准项目 Node 7 查询锚点 | 创建响应明确成功且本地对象 ID 完整时，Node 7 仅用官方列表接口的 `project_ids` 以十进制文本构造精确查询，并同时核验 ID 与 Draft 名称；名称过滤只用于创建响应不明且无 ID 的恢复性只读。每次回查创建独立脱敏 observation/evidence，按时间选择最新结果；空结果、ID 缺失或不一致均 fail-closed，绝不重发 create 或扩大五次/25 秒边界 | [本次批准 Task](../tasks/TASK-MWBV2-GAME-BRAND-FALLBACK-VALIDATION-20260910.md)、官方“获取标准项目列表”文档、[逻辑图](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
