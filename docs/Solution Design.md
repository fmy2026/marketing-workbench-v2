...
已确认的 `project_video_material_push` 只调用一次平台绑定。即时与后续恢复均按冻结 `video_ids` 查询目标账户；首次未可见时，Plan 保持已消费、Node 04 阻断，并提供“检查推送结果”的只读入口，不重新编制推送 Plan、不重新确认、不重发。每次观察以原 Job、Plan/hash、冻结批次与观察时间精确绑定，在同一事务保存脱敏 evidence、readback、Node 01–07 投影、Plan 和 Job；较早开始的迟到观察不得覆盖较新的结果。全部视频可见后，才重新核验项目素材并生成独立的追加确认卡。查询失败是未知状态，不能计为已核验或视频缺失。
