...
已批准 `TASK-MWBV2-MI-SEARCH-REPORT-20260920`：首版在同一市场情报工作区完成“提出问题 → 发现公共电脑当前候选中的真实游戏与素材 → 按研究对象和上海月份查找已采集素材 → 打开视频、趋势和证据解读 → 生成并编辑 HTML 月报”。发现阶段只读取一个受限分页候选，不表述为全量游戏目录；游戏名称只能来自公共电脑响应。查询只把每个研究对象当前最多 40 条候选素材的目标月份有效趋势作为归属依据，尚未遍历完时保留继续查询入口；浏览页每页 8 条。月报只采用有效观察样本，最多 30 条并按研究对象轮流选取，按“三个重点、研究对象、代表素材、继续关注”展示；可编辑摘要并导出无外部依赖的 HTML。模型可选：规则已识别的查询、确定性单素材证据说明和月报事实摘要无需模型；本人测试启用模型后，才额外处理未识别的自然语言或根据脱敏证据生成观察。数值、样本数、观察范围和证据关联始终由服务端计算。未配置或调用失败时保留确定性事实摘要，不建立情报库、报告历史或长期记忆。

市场情报入口沿用投放执行的 Agent 壳层：共享顶部栏、当前用户菜单、可收起的六模块导航和底部自然语言输入。默认“对话状态”只读取本人数据连接与模型配置，发送问题后才访问公共电脑；素材网格、详情、趋势、证据解读和月报都在对话工作区呈现。记忆和数据统计只消费当前页面状态，刷新后清空；知识库和技能只展示只读说明与入口，不新增业务数据或未实现能力。
...
数据仅从本人配置的公共电脑只读服务查询；连接地址和 Token 按工作台用户隔离，凭证保存于项目外 0600 文件，不回显、不下发长期 Token、不记录聊天原文或远端原始响应。服务地址只允许私网 IPv4 的 HTTP(S) origin，禁止 URL 凭据、路径与重定向；视频由后端验证身份并代理 GET/HEAD/Range。输入 Token 前显示 HTTP 明文传输说明，保存动作明确表示使用该连接进行只读验证。开发验收包括隔离模拟接口/双用户边界、浏览器页面、真实无凭证 401 与发布检查；真实带凭证数据及播放由用户录入后验证，单独说明，不能用模拟数据冒充。

关联 [Task](../tasks/TASK-MWBV2-MARKET-INTELLIGENCE-CHAT-20260918.md)。不修改投放创建执行链、平台权限或公共电脑配置，不迁移数据库。
