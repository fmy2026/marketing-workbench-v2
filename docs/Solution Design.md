...
| 确认创建跨执行轮次失败收口 | Skill 执行记录的唯一性以 `job_id + execution_cycle + skill_key + attempt_no` 为准；旧三字段约束必须移除。确认领取成功后若本地执行异常，只有同一 Job/Plan 经事务核验不存在任何平台 action、投递或创建对象时，才将 Plan 一次性消费为 `blocked_before_create` 并记录 `confirmed_create_execution_failed_before_action`；旧确认不可重放，随后只能经现有 fresh readonly recovery 生成新 Job、Plan/hash 并再次确认。任一 action、投递、对象或核验异常均保持关闭，转入既有回查或人工核验，不能推定零动作 | [本次批准 Task](../tasks/TASK-MWBV2-CREATE-CYCLE-FAILURE-CLOSURE-20260921.md)、[当前逻辑](project-现在的逻辑图.md)、[数据契约](project-数据与报表契约.md) |
