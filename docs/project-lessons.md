...
| --- | --- |
| 文档状态 | 当前有效；已验证可复用经验集 |
| 最后更新时间 | 2026-09-11 CST（目标账户空品牌列表省略、条件字段能力收敛、项目 ID 回查与乾坤素材码视频核验经验） |
...
| 追加成功统一收口 | 追加即时观察和用户发起的后续回查必须使用同一事务收口：readback、脱敏 evidence、节点 05–07、Job、Case 同步更新。不能将新建项目的对象 ID 投影借用于追加；成功标准仅是冻结视频集合在目标项目素材回查中全量命中。 |
...
| 绑定前预检 | JSZC-HUNT 的 10 条保底视频先按乾坤 `origin_resource_id` 精确查询素材库，必须 `10/10` 命中且 `resource_type=2`。物料户对账器按 `page_size=100` 扫完整个 `file/video/get` 库存；已验证案例为 `34` 页、`3,333` 条，每个来源码在 `filename` 中以大小写无关、完整边界唯一匹配返回项 `id`，只保存该 `id` 作为 `video_id`，避免 `4iG2-1` 误匹配 `4iG2-18`。乾坤预热记录 ID、状态和 `m_id` 是同步审计事实，`m_id` 永不作为 OceanEngine 视频 ID；库存唯一命中时，缺预热记录不否决当前物料户可用性。零/多文件名匹配、分页失败或来源类型错误均 fail-closed；预热接口只用于物料户未命中且存在异常既有记录的单次确认重发。 |
...
| 先分类、再组装 | 每个 fresh Job 先把目标账户品牌查询分类为：完整非空、**成功空列表**、失败/权限异常/结果不明。不得以旧账户页面、历史候选或游戏资产推断当前账户状态。 |
| 完整非空 | 只有目标账户的当前完整品牌和行业四字段同时可核验时，才发送完整 `brand_info`。 |
| 成功空列表 | 已完成一次受控真实验证：目标账户品牌列表 API 成功返回空列表时，创建接口可接受**整个省略**的 `brand_info`，且项目对象、名称和所需素材均已由权威只读回查确认。这个结论只适用于“新鲜成功空列表 + 当前 route/game/账户合同”的组合；它不证明任意账户都能省略品牌。 |
| 省略形态 | 省略必须是顶层 `brand_info` 完全不存在；禁止发送空对象、只删品牌名称/所属类别两个字段，或保留任一子字段。四个品牌字段是同一条件对象，必须一起发送或一起省略。 |
| 单一判定 | Node 04、Node 05、嵌套字段合同、payload、preflight、字段账本和 wire body 必须共用同一个 `brand_mode`；任何一个消费端仍要求旧的完整对象，都会造成“草稿已生成但不能确认”的伪阻断。 |
| 失败/不明 | 品牌接口失败、数据不完整、权限异常或结果不明时继续 `brand_info_not_ready`，不得把“查不到”理解为“可省略”。 |
| 历史候选 | 跨账户游戏维度品牌三元组只能作为失败诊断的历史证据，不能继承进新 Draft，更不能冒充目标账户授权。 |
| 写入边界 | `prepare_supported=false`；本 Node 不补写品牌或行业。 |
| 回查证据 | 保留目标账户品牌列表的脱敏只读证据、冻结 Draft 字段账本、单次 create action 与对象/名称/素材的权威回查；不保存原始请求或响应。 |
| 通用化结论 | 已完成成功创建与 Node 7 权威回查后，空列表规则已升级为通用能力：每个 fresh Job 重新查询目标账户，只有 API 成功且实际列表为零才整组省略 `brand_info`；无需 Case 特批、历史白名单或跨账户候选。非空未匹配、行业不完整、失败或不明仍严格阻断。 |

## 条件字段与账户能力差异：受控验证到通用能力的收敛（2026-09-11）

当官方接口将字段定义为“条件必填”，而不同目标账户的后台能力或只读结果存在差异时，不能仅凭历史账户、页面截图或未定位的错误码放宽全局合同。

| 阶段 | 可复用原则 |
| --- | --- |
| fresh 事实优先 | 每个 fresh Job 先读取当前目标账户的权威能力结果；运行时不得复用其他账户、历史 Case 或人工猜测作为字段来源。 |
| 受控验证的适用范围 | 现有合同无法覆盖一个有明确证据、但尚未验证的字段形态时，才可使用一次性、范围精确、可审计的验证。它必须绑定当前 Case、Job、字段差异、调用上限和本人确认，并继续复用既有 Plan、单次 executor 与权威回查；不得新增旁路写入入口，也不得把实验状态伪装为已验证能力。 |
| 最小字段差异 | 条件对象要么发送完整对象，要么按接口语义整体省略；不得通过只删除若干子字段、发送空对象或扩大其他字段省略范围来“兼容”。 |
| 成功后的收敛 | 只有创建成功且权威只读回查一致，才将结论收敛为“fresh 能力结果 → 稳定字段模式”，并删除运行时特批、白名单和历史候选依赖。 |
| 拒绝与不明 | 平台明确拒绝时保留失败证据、禁止自动重试或推广；结果不明时保持阻断，只允许只读排查。 |
| 长期 fail-closed | 查询失败、权限异常、结果不明、非空但不唯一匹配、数据不完整，或 payload/字段账本不一致时，均不得以“空值兼容”放行。 |

已验证实例：`oceanengine_std_project_create` 在目标账户品牌查询成功且实际返回空列表时，完整省略 `brand_info`；非空且唯一完整匹配时发送完整四字段 `brand_info`。该规则以每个 fresh Job 的目标账户查询为前提，不依赖 Case 特批或跨账户品牌候选。

## 标准项目 Node 7：ID 优先的权威回查（2026-09-10）

| 项 | 经验结论 |
| --- | --- |
| 查询锚点 | 创建响应明确成功且本地已保存对象 ID 时，只按官方列表接口的 `project_ids` 查询；名称仅用于同一记录的二次严格核验，不作为查询锚点。 |
| 大整数边界 | 平台项目 ID 必须作为已校验的十进制文本构造 `project_ids` 数组，禁止转换为 JavaScript `number`，避免精度损失。 |
| 名称过滤的边界 | 名称过滤可能持续返回空列表，即使对象已存在。只有创建响应不明且没有对象 ID 时，才允许它作为受控的恢复性只读查询；已确认响应却缺少本地 ID 必须 fail-closed，不得名称猜测。 |
| 通过标准 | 精确 ID、Draft 名称和所需素材绑定必须同时匹配；ID 或名称不一致、空列表或素材不完整均保持只读阻断，绝不补发 create。 |
| 观察留痕 | 每次 `readback_only` 使用独立 readback/evidence observation；历史“未命中”和最终 verified 都要保留，消费者只读取按时间最新的观察。 |
| 有界性 | 保留 `0/3/5/8/10` 秒、25 秒硬截止和至多 5 次只读调用。查询锚点错误不应通过延长等待或后台无限轮询修复。 |
