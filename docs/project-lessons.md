...
| 项 | 经验结论 |
| --- | --- |
| 归属与流转 | 先核验游戏/路线默认项，再核验目标账户可见性；不假定可跨账户直接复用。 |
| 只读判定 | 默认项存在且 active，HTTPS URL 仅以 hash 留证；目标账户可见、readback 通过、资产 ID 与 hash 一致。 |
| 写入边界 | `prepare_supported=false`；只读缺口不得自动补建或补写 URL。 |
| 回查证据 | 默认配置、目标资源状态、URL 存在性布尔值与 hash。 |
| 验证状态 | 当前为待验证资源准备路径。 |
| 不适用边界 | 不在文档、日志、API 或前端保存完整落地页 URL。 |
