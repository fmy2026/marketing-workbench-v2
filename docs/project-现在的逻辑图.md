...
`brand_info` 只接受目标账户的 fresh 品牌/行业回查。每个 fresh Job 由同一 `brandInfoMode` 判定三种稳定结果：非空列表中唯一完整匹配时为 `target_verified`，发送完整四字段对象；接口成功且平台实际返回列表数为零时为 `target_empty_omit`，整个省略 `brand_info`；查询失败、不明、非空未匹配/多匹配或行业不完整时为 `blocked`。空列表模式将品牌资源表示为 `not_required/not_required`，并必须保存当前 Job、列表数、响应 hash、查询时间与只读证据；它不依赖 Case 特批、账户白名单、游戏维度候选或历史 Plan。Node 04 的 `resourceReady` 是资源状态唯一判定：显式省略还必须同时具备有效只读证据；Node 05、嵌套字段合同和 Execution Plan 均复用它。空列表模式下嵌套合同、payload 和 preflight 共同要求整个 `brand_info` 顶层对象不存在；其他模式仍要求完整四字段对象。字段形态由单一条件字段合同消费：空列表模式只能具备精确的“对象＋四子字段”五条 omit 账本，函数仅在内存中将其投影为历史四字段发送形态并比较既有基线；品牌外形态、额外路径和残缺省略一律阻断，事实账本与请求不被改写。
