...
| 元信息 | 值 |
| --- | --- |
| 文档性质与状态 | 当前有效；运行流程、模块关系与实现定位 |
| 用途与范围 | 说明 Agent 如何从输入流转到受控结果；不定义字段口径或部署步骤 |
| 权威依据 | 当前 `src/`、`frontend/`、Agent 注册表、Workflow 注册表及数据契约 |
| 最后更新 | 2026-09-21 CST |
| 核验范围 | 静态核对 Agent 注册、服务入口、市场情报模块和当前代码路径；未做真实外部服务验收 |
| 更新条件 | Agent 职责、模块关系、公开入口、Workflow、权限流或实现定位变化时更新 |

> 本文不保存账户、Case、Job、Plan、资源、确认或平台动作状态。动态事实查 Postgres 或公共服务；字段、接口和数量限制查[数据与报表契约](project-数据与报表契约.md)，配置与发布操作查[部署说明](../deploy/README.md)。阅读入口见[文档导航](README.md)。
...
| 范围 | 当前入口 |
| --- | --- |
| Agent 注册与广场公开投影 | `src/agents/agentRegistry.mjs`、`frontend/app.js` |
| HTTP、认证、公共静态页和 Agent 路由 | `src/server/workbenchServer.mjs`、`src/security/` |
| 市场情报业务 | `src/agents/marketIntelligence*.mjs`、`src/platforms/marketIntelligenceClient.mjs`、`src/security/marketIntelligenceConnectionStore.mjs`、`frontend/market-intelligence.*` |
| 投放执行业务 | `src/agents/launch*.mjs`、`src/workflows/`、`src/platforms/oceanengine*.mjs` |

新增 Agent 可复用身份、网络边界、注册、模型配置和部署；其业务流程、外部数据适配和页面必须按自身合同建立。当前服务端和广场仍有市场情报、投放执行的专用分支，路由与目录通用化是后续实现事项。
...
- consumed Create Plan 的确认前停止与确认后的零动作本地异常只在确有 `blocked_before_create`、零 action、零 delivery 与零创建对象时进入该同一 readonly 恢复入口；通用 `readiness_not_ready:*`、授权探测包装原因不会覆盖 Skill 的具体传输或合同 blocker。Node 06 必须投影为受阻、Node 07 锁定，确认卡、提示和按钮都读取同一服务端 Gate/Plan/confirmation 可用性；确认被登记或 Plan 被消费后不再显示陈旧的可确认卡。
...
- 当前轮规则解析与规范化草稿合并后仍未完整时，模型才在本人已测试启用的配置下补槽位；确认、取消、状态和恢复始终规则优先。模型只能使用当前输入中可验证的证据；用户仅看到采用槽位名，或超时、供应商拒绝、非 JSON、意图/置信度、槽位/证据等受控回退分类，绝不显示或保存模型原始输出。服务端 `can_start` 与已校验请求快照是启动卡、顶部状态和点击入口的唯一可启动依据；模型回退只说明本轮解析，不额外阻断已校验的完整请求。
...
未指定游戏的问题进入候选发现，只显示本次公共服务响应中的游戏与素材。用户从已发现名称继续查询；按月结果和“已找到”都只表示当前加载候选，未完成时提供继续查询。素材详情与趋势每次重新读取；未知字段、上游失败或数据不足都停止结论。接口、字段、分页、样本数量以及零值、缺失和参考线的解释只查[市场情报外部只读合同](project-数据与报表契约.md#市场情报外部只读合同)。

### 解读与月报

规则负责已识别查询和确定性证据说明。本人测试并启用模型后，模型只能解析用户输入中的明确证据或改写已投影的脱敏素材证据；失败或不合格时回退规则说明。最终回复、素材解读和月报标明解析来源。月报只使用服务端重新核验的目标月份样本，固定展示重点、研究对象、代表素材和继续关注，可在当前页面编辑并导出离线 HTML。它不保存聊天、素材副本、报告历史或长期记忆，也不评价 ROI、排名或因果。

### 页面与权限

页面沿用六模块壳层；首屏只读取本人连接和模型配置，发送问题后才访问公共电脑。记忆和统计只投影当前页面状态，刷新即清空。市场情报不继承 Case、Gate、Plan、confirmation 或平台写权限；公共电脑负责采集、历史、质量校验和指标计算。连接配置与真实联调操作查[部署说明](../deploy/README.md#市场情报数据连接)。
