...
|---|---|---|
| 目标与方案 | 方案设计 → [Solution Design.md](Solution%20Design.md) | 做什么、做到什么程度、为什么这样做；记录人决定的关键取舍和验收标准 |
| 整体逻辑 | 整体逻辑图 → [当前逻辑图](project-现在的逻辑图.md) | 输入如何成为结果；标出模块输入输出、变化点、模型与规则分工、人工决策和失败处理 |
| 知识准备 | 知识索引与权威资料 → [资料入口](Solution%20Design.md#资料使用) | 为关键判断提供依据；标明来源、版本、适用范围和待核实项 |
| 数据与报表 | 数据与报表设计 → [数据契约](project-数据与报表契约.md) | 明确实体关系、记录粒度、归属、来源、读写责任和历史；约定独立统计与组合分析口径 |
...
**推进顺序：明确结果 → 画最小闭环 → 核准知识与数据 → 确定方案 → 按任务实现和验证 → 更新整体逻辑 → 用第二场景检验扩展。**
