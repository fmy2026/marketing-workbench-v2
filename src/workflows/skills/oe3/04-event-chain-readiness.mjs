...
function microAppInstanceBindingReadbackVerified(bundle = {}) {
...
function routeQuery(bundle = {}, { assetId = "", instanceId = "" } = {}) {
  ...
  return {
    query: {
      ...
      mini_program_id: microAppInstanceCandidate(bundle).appId,
...
function microAppInstanceAuthorityContract({ status, blockers = [], instance, probe, goalSummary = {}, evidenceRef = "" } = {}) {
  return sanitizeForPublic({
    ...
    // This probe is diagnostic only. It must never promote the instance to
    // target-account verified or unblock an event-asset Plan.
    diagnostic_passed: status === "passed",
    target_instance_readback_verified: false,
...
export async function runMicroAppInstanceAuthorityReadonlySkill({
  ...
  if (mockReady) {
    return sanitizeForPublic({
      ...
      blockers: [],
      outputSummary: {
      targetInstanceReadbackVerified: false,
  ...
  if (!permission.allowed) {
    return sanitizeForPublic({
      ...
      outputSummary: { targetInstanceReadbackVerified: false, diagnosticPassed: false, platformWriteCalled: false, tokenRefreshCalled: false, rawRequestStored: false, rawResponseStored: false }
    });
  }
  ...
  const result = sanitizeForPublic({
    ...
    outputSummary: {
      ...
      diagnosticPassed: contract.diagnostic_passed === true,
...
function eventContractFor({
  ...
  return sanitizeForPublic({
    ...
    target_instance_candidate_trusted: instance.instanceCandidateTrusted === true,
    target_instance_binding_readback_verified: instanceBindingVerified === true,
    target_instance_readback_verified: instanceBindingVerified === true,
...
function classifyEventCandidate({ inventoryResult, details, appId, instanceId = "" }) {
  ...
  if (!appId || !appBindingObservable || !appBound.length) {
...
async function persistEventChain({ repo, bundle, status, blockers, contract, candidate }) {
  ...
  const identityVerified = contract.event_asset_identity_readback_verified === true;
  const instanceBindingVerified = contract.target_instance_binding_readback_verified === true;
  const instance = microAppInstanceCandidate(bundle);
  await repo.updateAccountResourceReadonly({
    ...
    visibilityStatus: identityVerified ? "visible" : undefined,
    readbackStatus: identityVerified ? "readback_verified" : undefined,
    platformResourceId: identityVerified ? candidate?.id || "" : undefined,
    inheritanceStatus: identityVerified ? "target_readonly_verified" : "target_readonly_blocked",
  ...
  await repo.updateAccountResourceReadonly({
    ...
    visibilityStatus: instanceBindingVerified ? "visible" : undefined,
    readbackStatus: instanceBindingVerified ? "readback_verified" : undefined,
    platformResourceId: instanceBindingVerified ? instance.instanceId : undefined,
    inheritanceStatus: instanceBindingVerified ? "target_readonly_verified" : "target_readonly_blocked",
    metadata: baseReadonly,
    resourceMetadata: { event_chain_readonly_contract: contract }
  });
}
...
export function eventChainResourceReadiness({ bundle = {}, resourceType }) {
  ...
  const contract = item.metadata?.event_chain_readonly_contract || {};
  const candidate = microAppInstanceCandidate(bundle);
  const candidateBlockers = microAppInstanceCandidateBlockers(candidate);
  ...
  const eventAssetPreparePending = resourceType === "event_asset" && !passed &&
    contract.event_asset_identity_readback_verified !== true &&
    provision?.outputSummary?.planEligible === true;
  const microPendingStage = resourceType === "micro_app_instance" && !passed && !candidateBlockers.length
    ? contract.target_instance_binding_readback_verified === true
      ? "waiting_on_event_configs"
      : "waiting_on_event_asset"
    : "";
  const blockers = passed || eventAssetPreparePending || microPendingStage
    ? []
    : [...new Set([
      ...(candidateBlockers.length ? candidateBlockers : contract.blocker_codes?.length ? contract.blocker_codes : [fallback]),
      ...(provision?.blockers || [])
    ])];
  const readinessStatus = passed ? "ready" : eventAssetPreparePending ? "prepare_supported" : microPendingStage || "blocked";
  return sanitizeForPublic({
    // launch_skill_runs has a closed status enum. Transitional resource
    // progress remains in prepareCapability/readinessStatus, while the Skill
    // record stays blocked until the full resource is READY.
    ...
    outputSummary: {
      ...
      targetInstanceReadbackVerified: microAppInstanceBindingReadbackVerified(bundle),
      targetInstanceCandidateTrusted: candidate.instanceCandidateTrusted === true,
      ...
      ...(provision ? {
        ...
        prepareCapability: {
          status: eventAssetPreparePending ? "prepare_supported" : passed ? "ready" : "blocked",
          prepare_action_type: eventAssetPreparePending ? "ensure_resource:event_asset" : ""
        },
        prepare_capability: {
          status: eventAssetPreparePending ? "prepare_supported" : passed ? "ready" : "blocked",
          prepare_action_type: eventAssetPreparePending ? "ensure_resource:event_asset" : ""
        ...
        prepareCapability: { status: microPendingStage },
        prepare_capability: { status: microPendingStage }
      } : {}),
      nextAction: passed
        ? "无需动作"
        : eventAssetPreparePending
          ? "可生成事件资产与 baseline 配置的确认 Plan。"
          : microPendingStage === "waiting_on_event_asset"
            ? "等待事件资产详情确认 App 与实例绑定。"
            : microPendingStage === "waiting_on_event_configs"
              ? "等待 baseline 事件配置完成后进行优化目标与 DBT 回查。"
              : "完成目标账户事件资产、小游戏实例与优化目标链准备后重新只读核验。"
    }
  });
}
...
export async function runEventChainReadonlySkill({
  ...
  const instance = microAppInstanceCandidate(bundle);
  const candidatePreflightBlockers = [
    ...(!instance.appId || instance.appType !== "byte_mini_game" || instance.appStatus !== "active"
      ? ["micro_app_platform_app_unverified"]
      : []),
    ...microAppInstanceCandidateBlockers(instance)
  ];
  const inventoryResult = candidatePreflightBlockers.length
    ? { status: "skipped", blockers: [], probes: [], items: [] }
    : await readInventory({ bundle, client });
  ...
  const event = candidatePreflightBlockers.length
    ? {
        candidate: null,
        blockers: [],
        inventory: {
          pageCount: 0,
          candidateCount: 0,
          appBoundCandidateCount: 0,
          instanceBoundCandidateCount: 0,
          instanceBindingObservable: false,
          appBindingObservable: false,
          candidateSelectionSource: ""
        }
      }
  ...
  const status = blockers.length ? "blocked" : "passed";
  const instanceBindingVerified = identityVerified && event.inventory.instanceBindingObservable === true &&
    Number(event.inventory.instanceBoundCandidateCount || 0) === 1;
